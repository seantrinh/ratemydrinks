...
router.post("/", async (req, res) => {
	...
	if(req.session.user){
		try {
			...
			res.render("layouts/search", { heading: "Find A Drink", drinks: result, USERNAME: req.session.user, USERAUTH: true});
		} catch (e) {
...
router.get("/",async(req,res)=>{
	if(req.session.user){
		try{
			res.render("layouts/search", { heading: "Find A Drink",drinks: 5, USERNAME: req.session.user, USERAUTH: true});
        } catch (e) {
                res.status(400).render('layouts/error', { heading: "400 Error" });
        }
	} else{
